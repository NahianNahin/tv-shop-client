...
const ReportedItems = () => {
    ...
    const { data: products = [], isLoading, refetch } = useQuery({
        queryKey: ['products'],
        queryFn: async () => {
            const res = await fetch(`https://my-assignment-12-server-nahiannahin.vercel.app/reported_products`, {
                headers: {
    ...
    const handleDeleteProduct = id => {
        fetch(`https://my-assignment-12-server-nahiannahin.vercel.app/product/${id}`, {
            method: 'DELETE',
            headers: {
                authorization: `bearer ${localStorage.getItem('TV_Shop_Token')}`
            }
        ...
        <div className='p-20'>
            {
                products.length === 0
                    ?
                    <p className='text-3xl text-error font-bold'> No Reported Items Available</p>
                    :
                    <div >
                        <p className='text-3xl'>All Reported Items</p>
